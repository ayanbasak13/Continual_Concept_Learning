...
def generate_gmm_samples(size, components, dimensions, means, covariance, weights):
    """Generates synthetic data of a given size from a random Gaussian Mixture Model"""
    # np.random.seed(seed)
...
def sample_gmm(num_samples, COMPONENTS, DIMENSIONS, task=1) :

    final_means = np.load('/Users/ayanbask/PycharmProjects/VAE/results/latent_vectors_GMM_means_t' + str(task) + '.npy', allow_pickle=True)
    final_covariances = np.load('/Users/ayanbask/PycharmProjects/VAE/results/latent_vectors_GMM_covariances_t' + str(task) + '.npy', allow_pickle=True)
    final_weights = np.load('/Users/ayanbask/PycharmProjects/VAE/results/latent_vectors_GMM_weights_t' + str(task) + '.npy', allow_pickle=True)

    samples = generate_gmm_samples(num_samples, COMPONENTS, DIMENSIONS, final_means, final_covariances,
                                                final_weights)
